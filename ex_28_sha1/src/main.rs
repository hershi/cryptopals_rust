...
fn pad(mut message: Vec<u8>) -> Vec<u8> {
    ...
    println!("message len with footer: {}", message_len_with_footer);
    println!("bits in last block: {}", bits_in_last_block);
...
fn sha1(message: &[u8]) -> Vec<u32> {
    println!("AAAMessage Len: {}", message.len());
    let message = pad(message.to_vec());
    println!("BBBMessage Len: {}", message.len());
...
fn main() {
    let w = 0xABCDEF45;
    let shift = 2;
    println!("S^{}({:032b}) == {:032b})", shift, w, circular_shift(w, shift));

    let x = f(21, 0xFF, 0xAA, 0xFF);
    println!("{:x} {:032b}", x ,x);
